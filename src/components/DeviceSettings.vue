<template>
  <v-card title="Device settings" style="display: flex; flex-direction: column; align-items: center; width: 100%;">
    <v-btn :disabled="true" style="display: flex; justify-content: space-evenly; width: 80%; margin: 1%;"
      @click="toggleAddNewDevice">
    ...
    <v-card v-if="addNewDevice" style="display: flex; flex-direction: column; align-items: center; width: 80%;">
      <v-text-field v-model="clientID" label="Client ID" required hide-details style="width: 100%;">
      </v-text-field>
      <v-text-field v-model="clientSecret" label="Client Secret" required hide-details style="width: 100%;">
      </v-text-field>
      <v-text-field v-model="serialNumber" label="Serialnumber" required hide-details style="width: 100%;">
      </v-text-field>
      <v-btn style="margin: 2%;" @click="addDevice">Add device</v-btn>
    </v-card>
    <v-btn :disabled="true" style="display: flex; justify-content: space-evenly; width: 80%; margin: 1%;"
      @click="toggleEditDevice">
    ...
    <v-card v-if="editValues" style="display: flex; flex-direction: column; align-items: center; width: 80%;">
      <v-text-field type="number" v-model="pricelimit" label="Price limit for electricity in Øre" required hide-details
        style="width: 100%;">
      </v-text-field>
