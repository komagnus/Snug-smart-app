<template>
  ...
  <v-card class="cardStyle">
    <v-slider v-model="temp" min="15" max="30" color="green" style="width: 60%;margin-bottom: 20%;">
    </v-slider>
  ...
  <v-dialog v-model="showMoreDeviceData" persistent width="80%">
    <v-card style="width: 100% ;display: flex; justify-content: space-evenly; flex-direction: column; margin-top: 1%;">
      <v-col @click="toggleShowMoreDeviceData" style="align-self: flex-end; cursor: pointer;">
        <v-icon size="large">
          mdi-close
        </v-icon>
...
watch(() => temp.value, (newValue) => {
  if (formattedTemp.value) {
    console.log(formattedTemp.value)
  }
});
